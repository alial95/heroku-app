...
from .forms import AreaForm, NationForm
import json
...
def covid_home(request):
    ...
    def make_graph(data):
        ...
        x = reverse_list([x['date'] for x in data[:30]]) # flip the data around to see it ascending by date
        y1 = reverse_list([x['dailyCases'] for x in data[:30]])
...
def nation_select(request):
    ...
    if request.method == 'POST':
        area_id = request.POST['model_choice']  # get the area pk from the form
        area_name = Regions.objects.get(pk=area_id)
    ...
    return render(request, 'covid_nations.html', context)
